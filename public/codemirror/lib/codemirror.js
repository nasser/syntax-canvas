...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function measureLine(line, ch) {
      if (ch == 0) return {top: 0, left: code.clientWidth};
      var wbr = options.lineWrapping && ch < line.text.length &&
                spanAffectsWrapping.test(line.text.slice(ch - 1, ch + 1));
      measure.innerHTML = "<pre>" + line.getHTML(makeTab, ch, tempId, wbr) + "</pre>";
      var elt = document.getElementById(tempId);
      var top = elt.offsetTop, left = elt.offsetLeft, right = elt.offsetLeft + elt.offsetWidth;
    ...
    function coordsChar(x, y) {
      ...
      function getX(len) {
        ...
        if (tw) {
          ...
          return Math.min(code.clientWidth, sp.right + (off - innerOff) * scroller.clientWidth);
        }
        return sp.right;
      }

      var from = 0, fromX = code.clientWidth, to = text.length, toX;
      // Guess a suitable upper bound for our search.
